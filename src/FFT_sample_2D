...
#define MAX_MILLIAMPS   450          // Careful with the amount of power here if running off USB port
const int BRIGHTNESS_SETTINGS[3] = {25, 7, 50};  // 3 Integer array for 3 brightness settings (based on pressing+holding BTN_PIN)
...
void loop() {
  ...
  FastLED.clear(); // see if there is a better way to do this
  ...
  for (byte band = 0; band < NUM_BANDS; band++) {
    ...
    if (VERBOSE) {
      char buffer[50];
      sprintf(buffer,"bh[%d]:%d ", band, barHeight);
      Serial.print(buffer);
    }
...
/*
  for (int i = 0; i <= Q1; i++) {
    leds[   i] = CRGB::Red;
    leds[Q2-i] = CRGB::Blue;
    leds[Q2+i] = CRGB::Yellow;
    leds[Q4-i] = CRGB::Green;
    delay(25);
    FastLED.show();
  }
  delay(500);
...
void purpleBars(int band, int barHeight) {
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++) {
    for (int y = TOP; y >= TOP - barHeight; y--) {
      ...
      //Serial.println(ColorFromPalette(purplePal, y * (255 / (barHeight + 1))).r);
      //matrix->drawPixel(x, y, CRGB::Blue);
    }
  }
}
